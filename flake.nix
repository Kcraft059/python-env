      ...
      devShells."${system}".default =
        ...
        pkgs.mkShell {
          ...
          shellHook = ''
            ...
            if [[ ! -f "$(readlink -f ./.venv/bin/python)" ]] && [[ -d ./.venv ]]; then 
            ...
            [[ -d ./.venv ]] || ( python -m venv .venv && echo -e "$smak Created venv" )

            pydir=$(ls ${python3env}/lib/ | grep "^python")

            # Clean old links
            for file in $(find .venv/lib/''${pydir}/site-packages -maxdepth 1 -type l); do # Find all links in site-packages
            ...
            for file in ${python3env}/lib/''${pydir}/site-packages/*; do
              ln -s $file ./.venv/lib/''${pydir}/site-packages/ 1>/dev/null 2>&1 
            done
